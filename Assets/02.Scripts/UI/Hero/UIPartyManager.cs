...
namespace AFKHero.UI
{
    public class UIPartyManager : MonoBehaviour
    {
        ...
        [SerializeField] private Image[] partyHeroImages;  // 영웅 이미지 -> 일단 이미지로

        [Header("시너지")]
        [SerializeField] private UISynergy synergyUI;
        ...
        public void UpdatePartySet()
        {
            for (int i = 0; i < partyHeroImages.Length; i++)
            {
                ...
                {
                    partyHeroImages[i].gameObject.SetActive(false);
                    continue;
                }

                partyHeroImages[i].gameObject.SetActive(true);
                partyHeroImages[i].sprite = hero.data.HeroIcon;
            }
        }
